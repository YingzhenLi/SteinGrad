...
index_features = np.array(range(0, data.shape[ 1 ] - 1), dtype = int)
...
# We store the number of classes

np.savetxt("n_classes.txt", \
    np.array([ len(set(data[ : , index_target ].T.tolist()[ 0 ])) ]), \
    fmt = '%d')
