...
def Matern_kernel(X, Y, sigma=1.0):
    # compute Matern 3/2 kernel with 3sigma^2 = rho^2
    assert X.shape[1] == Y.shape[1]
    p_dists = scipy.spatial.distance.cdist(X, Y, 'euclidean')
    K_laplace = np.exp(-p_dists / sigma)
    K_matern = sigma**2 * (1 + p_dists / sigma) * K_laplace
    
    return K_matern, K_laplace
...
class KernelExpStein(EstimatorBase):
    def __init__(self, sigma, lmbda, D, N, beta = -0.5):
        self.sigma = sigma
        self.lmbda = lmbda
        self.D = D
        self.N = N
    ...
    def fit_wrapper_(self):
        Km, Kl = Matern_kernel(self.X, self.X, sigma=self.sigma)	 # shape (K, K)
        self.K_inv = np.linalg.inv(Km + self.lmbda * np.eye(Km.shape[0]))	# shape (K, K)
        # here self.sigma equiv to 2*sigma**2 in the paper
        sumKl = np.sum(Kl, axis=1)[:, np.newaxis]	# shape (K, 1)
        grad = sumKl * self.X - np.dot(Kl, self.X)
        self.X_grad = -np.dot(self.K_inv, grad)
