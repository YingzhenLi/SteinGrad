...
def compute_b_and_C(X, Y, K_XY, sigma):
    ...
    K = K_XY
    KK = K * K	# element-wise product
    KK_K = np.dot(KK, K)
        
    b = np.dot(KK, np.dot(np.diag(np.diag(XY)), KK)) + np.dot(KK, KK) * XY \
                - np.dot(KK * XY, KK) - np.dot(KK, KK * XY)
    b = np.sum(b, 1)	# shape (K)
    C = XY * np.dot(KK_K, KK) + np.dot(KK, np.dot(K * XY, KK)) \
                - np.dot(KK_K * XY, KK) - np.dot(KK, KK_K.T * XY)	# shape (K, K)
...
class KernelExpStein(EstimatorBase):
    ...
    def fit_wrapper_(self):
        self.K = Cauchy_kernel(self.X, self.X, sigma=self.sigma)
